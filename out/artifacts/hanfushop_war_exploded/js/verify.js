...
$(document).ready(function() {
	$('#register-account-div').find('#register-account').change(function() {
		if (regAccount.test($(this).val())) {
			...
			fail($("#register-account-div"), '用户名太短，不能少于3个字符');
...
$(document).ready(function() {
	$('#register-email-div').find("#register-email").change(function() {
		if (regEmail.test($(this).val())) {
			...
			fail($("#register-email-div"), '邮箱格式错误,正确格式如:123456@scut.edu.cn');
	...
	$('#modified-email-div').find("#modified-email").change(function() {
		if (regEmail.test($(this).val())) {
			...
			fail($("#modified-email-div"), '邮箱格式错误,正确格式如:123456@scut.edu.cn');
...
var checkRegisterAddress = false;
var checkModifiedAddress = true;
$(document).ready(function() {
	$('#register-address-div').find("#register-address").change(function() {
		if (regAddress.test($(this).val())) {
			success($("#register-address-div"));
			checkRegisterAddress = true;
		} else {
			fail($("#register-address-div"), '地址格式错误');
			checkRegisterAddress = false;
		}
	});
	$('#modified-address-div').find("#modified-address").change(function() {
		if (regAddress.test($(this).val())) {
			success($("#modified-address-div"));
			checkModifiedAddress = true;
		} else {
			fail($("#modified-address-div"), '地址格式错误');
			checkModifiedAddress = false;
		}
	});
});
...
function checkRegister() {
	...
	if (checkRegisterAddress == true) {
		success($("#register-address-div"));
	} else {
		fail($("#register-address-div"));
	}
	...
	if (checkRegisterAccount == true && checkRegisterPassword == true
			...
			&& checkRegisterEmail == true && checkRegisterAddress == true && checkIdcode == true) {
		return true;
	} else {
...
function checkModifyInfo() {
	...
	if (checkModifiedAddress == true) {
		success($("#modified-address-div"));
	} else {
		fail($("#modified-address-div"));
	}

	if (checkModifiedTel == true && checkModifiedEmail == true &&checkModifiedAddress == true) {
		return true;
	} else {
		event.preventDefault();
		return false;
